  ...
  return (
    <div className="space-y-6 pb-8 font-sans">
      ...
      <div className="border-b border-slate-800/80 pb-4">
        <div className="flex items-center space-x-2">
          <h2 className="text-xl font-bold font-mono text-slate-100 tracking-tight uppercase">
            NETWORK TRAFFIC INTELLIGENCE & ANALYTICS
          </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800/80 border-t-4 border-t-cyan-400 p-4 rounded-xl shadow-xl hover:border-cyan-500/50 transition-all">
          ...
          <p className="text-2xl font-mono font-bold text-slate-100">
            {traffic ? traffic.total_packets_sec.toFixed(1) : '0.0'} <span className="text-xs text-slate-400">pkt/s</span>
          </p>
        </div>

        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800/80 border-t-4 border-t-indigo-400 p-4 rounded-xl shadow-xl hover:border-indigo-500/50 transition-all">
          <div className="flex justify-between items-center text-slate-400 text-xs font-mono mb-2">
        ...
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800/80 border-t-4 border-t-purple-400 p-4 rounded-xl shadow-xl hover:border-purple-500/50 transition-all">
          <div className="flex justify-between items-center text-slate-400 text-xs font-mono mb-2">
        ...
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800/80 border-t-4 border-t-emerald-400 p-4 rounded-xl shadow-xl hover:border-emerald-500/50 transition-all">
          <div className="flex justify-between items-center text-slate-400 text-xs font-mono mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-xl p-5 shadow-xl space-y-4">
          <h3 className="text-sm font-bold text-slate-100 tracking-tight flex items-center gap-2 border-b border-slate-800/80 pb-3 uppercase font-mono">
          ...
          {traffic && traffic.protocol_distribution ? (
            <div className="space-y-3 font-mono text-xs">
              {Object.entries(traffic.protocol_distribution).map(([proto, count]) => {
                ...
                return (
                  <div key={proto} className="space-y-1.5 bg-slate-950/60 p-3 rounded-lg border border-slate-800/80">
                    <div className="flex justify-between text-xs">
                      <span className="font-bold text-cyan-400">{proto}</span>
                      <span className="text-slate-300 font-mono">{count.toLocaleString()} pkts ({pct}%)</span>
                    </div>
            ...
            <div className="text-slate-400 font-mono text-xs py-8 text-center bg-slate-950/60 rounded-lg border border-dashed border-slate-800">
        ...
        <div className="lg:col-span-7 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-xl p-5 shadow-xl flex flex-col justify-between">
          <h3 className="text-sm font-bold text-slate-100 tracking-tight flex items-center gap-2 border-b border-slate-800/80 pb-3 uppercase font-mono">
            <Server className="h-4 w-4 text-emerald-400" />
            ...
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={history}>
                  ...
                  <Area type="monotone" dataKey="tcp_packets" stroke="#06B6D4" strokeWidth={2} fill="#06B6D4" fillOpacity={0.15} name="TCP Packets" />
                  <Area type="monotone" dataKey="udp_packets" stroke="#7C3AED" strokeWidth={2} fill="#7C3AED" fillOpacity={0.15} name="UDP Packets" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-slate-400 text-xs font-mono border border-dashed border-slate-800 bg-slate-950/60 rounded-lg">
